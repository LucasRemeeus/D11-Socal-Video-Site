...
$getdata = $mysqli -> prepare("SELECT ProfilePicture, Username FROM user where ID_User =?");
...
if(isset($_POST["form_submit"])) {
    ...
    if($imageProcess == 1){
        ...
        $DatabaseUpdate = $mysqli -> prepare("UPDATE `user` SET `ProfilePicture` = ? WHERE `user`.`ID_User` = ?");
